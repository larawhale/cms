...
| `config.input_attributes` | array | An array that will be passed on to the input that is being rendered. This can be useful to add a `placeholder`, `checked`, `class` or other native html attributes. |
...
It is possible to create your own custom field classes. You might for example wish to render a more complex input field in the user interface or do some additional things during the saving to the database.
